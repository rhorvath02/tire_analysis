...
scaling_coeffs[8:15] = [0.979123173690743, 1.0223873328170323, 1.2236120016602519, 1.5248039098784438, 0.40683312470208527, 0.25047510437176024, 0.8202232715731637]

pure_lat_coeffs = [1.3527376401722393, -2.5036312664108062, 0.6505973463169893, 9.423768050812576, 0.17849509152594265, -0.04752524063648042, 0.8079559806081511, 42.81076780141361, 31.076122612890995, 1.2651199149814782, 0.6105325571031571, -0.0025491228717106594, 0.001096211018526508, -0.06944567882483914, 0.012229623569108243, -0.018945437981311805, -1.4250325348174189, -1.0850605408574026]
...
x2_lst = (df2["FZ"] * -1).tolist()[::50]
y2_lst = (df2["SA"] * np.pi / 180).tolist()[::50]
z2_lst = (df2["IA"] * np.pi / 180).tolist()[::50]

w2_lst = (df2["FY"] * -1).tolist()[::50]
...
model_x_data = np.linspace(2000, abs(min(x1_lst)), 1000)
...
Z2 = _pure_lat([350 * 4.44822, X, Y, 0], pure_lat_coeffs)
...
# ax.scatter3D(x1_lst, y1_lst, w1_lst, color='red')
# ax.scatter3D(x2_lst, y2_lst, w2_lst, color='blue')
