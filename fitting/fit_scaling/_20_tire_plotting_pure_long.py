...
pure_long_coeffs = [1.351280954310543, 2.2476529305423965, -0.8509484823458834, 12.336123849123595, -0.5526590796664804, -1.4566689238399961, -0.04434258610672221, -0.36609019329529896, 48.73023518936532, 34.8045614586387, -1.0992123979938666, -0.003292949625684342, -0.006907553737528136, -0.08734932112093803, -0.04704948793424146]

# pure_long_coeffs = [1.2930625586160691, 2.211229249110004, -0.16798288319407825, 12.181101396052222, -1.64695549112698, -2.2395931575430614, 0.1576341778627693, -0.33850481120565357, 38.193424775285166, 29.248684981264578, -1.5085159905680003, -0.0013842147344984934, -0.0016646763853847, -0.08734932112093803, -0.04704948793424146]
...
tires = {"Hoosier_20.5x7.0-13_R20_7_braking":{"long":None, "lat":None}}
...
for name, tire in tires.items():
    try:
        ...
        tire["long"] = df[(df["pressure"] == pressure) & (df["velocity"] == velocity) & (df["camber"] == 0) & (df["slip"] == 0)]
        # print(tire["long"])
...
x_lst = (df["load"] * -1).tolist()[::25]
y_lst = (df["SL"]).tolist()[::25]
z_lst = (df["camber"] * np.pi / 180).tolist()[::25]

w_lst = (df["FX"]).tolist()[::25]
...
# model_y_data = np.linspace(-0.2, 0.2, 1000)
